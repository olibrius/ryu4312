...
class Test_cfm(unittest.TestCase):
    ...
    def test_init(self):
        assert str(self.message) == str(self.ins.op)
    ...
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert str(self.message) == str(res.op)
    ...
    def test_to_string(self):
        ...
        assert str(self.ins) == cfm_str
        assert repr(self.ins) == cfm_str
    ...
    def test_len_cc_message(self):
        self.setUp_cc_message()
        assert len(self.ins) == 0 + len(self.message)
...
class Test_cc_message(unittest.TestCase):
    ...
    def test_init(self):
        assert self.md_lv == self.ins.md_lv
        assert self.version == self.ins.version
        assert self.rdi == self.ins.rdi
        assert self.interval == self.ins.interval
        assert self.seq_num == self.ins.seq_num
        assert self.mep_id == self.ins.mep_id
        assert self.md_name_format == self.ins.md_name_format
        assert self.md_name_length == self.ins.md_name_length
        assert self.md_name == self.ins.md_name
        assert self.short_ma_name_format == self.ins.short_ma_name_format
        assert self.short_ma_name_length == self.ins.short_ma_name_length
        assert self.short_ma_name == self.ins.short_ma_name
        assert self.tlvs == self.ins.tlvs
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.md_lv == res.md_lv
        assert self.version == res.version
        assert self.rdi == res.rdi
        assert self.interval == res.interval
        assert self.seq_num == res.seq_num
        assert self.mep_id == res.mep_id
        assert self.md_name_format == res.md_name_format
        assert self.md_name_length == res.md_name_length
        assert self.md_name == res.md_name
        assert self.short_ma_name_format == res.short_ma_name_format
        assert self.short_ma_name_length == res.short_ma_name_length
        assert self.short_ma_name == res.short_ma_name
    ...
    def test_parser_with_no_maintenance_domain_name_present(self):
        ...
        assert self.seq_num == res.seq_num
        assert self.mep_id == res.mep_id
        assert cfm.cc_message._MD_FMT_NO_MD_NAME_PRESENT == res.md_name_format
        assert self.short_ma_name_format == res.short_ma_name_format
        assert self.short_ma_name_length == res.short_ma_name_length
        assert self.short_ma_name == res.short_ma_name
        assert self.tlvs == res.tlvs
    def test_serialize(self):
        ...
        assert self.rdi == res[2] >> 7
        assert self.interval == res[2] & 0x07
        assert self.first_tlv_offset == res[3]
        assert self.seq_num == res[4]
        assert self.mep_id == res[5]
        assert self.md_name_format == res[6]
        assert self.md_name_length == res[7]
        assert self.md_name == res[8]
        assert self.short_ma_name_format == res[9]
        assert self.short_ma_name_length == res[10]
        assert self.short_ma_name == res[11]
        assert self.end_tlv == res[12]
    ...
    def test_serialize_with_no_maintenance_domain_name_present(self):
        ...
        assert self.seq_num == res[4]
        assert self.mep_id == res[5]
        assert cfm.cc_message._MD_FMT_NO_MD_NAME_PRESENT == res[6]
        assert self.short_ma_name_format == res[7]
        assert self.short_ma_name_length == res[8]
        assert self.short_ma_name == res[9]
        assert self.end_tlv == res[10]
    def test_len(self):
        # 75 octet (If tlv does not exist)
        assert 75 == len(self.ins)
    def test_default_args(self):
        ins = cfm.cc_message()
        buf = ins.serialize()
...
class Test_loopback_message(unittest.TestCase):
    ...
    def test_init(self):
        assert self.md_lv == self.ins.md_lv
        assert self.version == self.ins.version
        assert self.transaction_id == self.ins.transaction_id
        assert self.tlvs == self.ins.tlvs
    def test_parser(self):
        ...
        assert self.md_lv == res.md_lv
        assert self.version == res.version
        assert self.transaction_id == res.transaction_id
        assert self.tlvs == res.tlvs
    def test_serialize(self):
        buf = self.ins.serialize()
        res = struct.unpack_from(self.form, bytes(buf))
        ...
        assert self.version == res[0] & 0x1f
        assert self.opcode == res[1]
        assert self.flags == res[2]
        assert self.first_tlv_offset == res[3]
        assert self.transaction_id == res[4]
        assert self.end_tlv == res[5]
    def test_len(self):
        # 9 octet (If tlv does not exist)
        assert 9 == len(self.ins)
    def test_default_args(self):
...
class Test_link_trace_message(unittest.TestCase):
    ...
    def test_init(self):
        assert self.md_lv == self.ins.md_lv
        assert self.version == self.ins.version
        assert self.use_fdb_only == self.ins.use_fdb_only
        assert self.transaction_id == self.ins.transaction_id
        assert self.ttl == self.ins.ttl
        assert self.ltm_orig_addr == self.ins.ltm_orig_addr
        assert self.ltm_targ_addr == self.ins.ltm_targ_addr
        assert self.tlvs == self.ins.tlvs
    def test_parser(self):
        ...
        assert self.md_lv == res.md_lv
        assert self.version == res.version
        assert self.use_fdb_only == res.use_fdb_only
        assert self.transaction_id == res.transaction_id
        assert self.ttl == res.ttl
        assert self.ltm_orig_addr == res.ltm_orig_addr
        assert self.ltm_targ_addr == res.ltm_targ_addr
        assert self.tlvs == res.tlvs
    def test_serialize(self):
        buf = self.ins.serialize()
        res = struct.unpack_from(self.form, bytes(buf))
        ...
        assert self.version == res[0] & 0x1f
        assert self.opcode == res[1]
        assert self.use_fdb_only == res[2] >> 7
        assert self.first_tlv_offset == res[3]
        assert self.transaction_id == res[4]
        assert self.ttl == res[5]
        assert addrconv.mac.text_to_bin(self.ltm_orig_addr) == res[6]
        assert addrconv.mac.text_to_bin(self.ltm_targ_addr) == res[7]
        assert self.end_tlv == res[8]
    def test_len(self):
        # 22 octet (If tlv does not exist)
        assert 22 == len(self.ins)
    def test_default_args(self):
        ...
        assert res[4] == 0
        assert res[5] == 64
        assert res[6] == addrconv.mac.text_to_bin('00:00:00:00:00:00')
        assert res[7] == addrconv.mac.text_to_bin('00:00:00:00:00:00')
class Test_link_trace_reply(unittest.TestCase):
    ...
    def test_init(self):
        ...
        assert self.version == self.ins.version
        assert self.use_fdb_only == self.ins.use_fdb_only
        assert self.fwd_yes == self.ins.fwd_yes
        assert self.terminal_mep == self.ins.terminal_mep
        assert self.transaction_id == self.ins.transaction_id
        assert self.ttl == self.ins.ttl
        assert self.relay_action == self.ins.relay_action
        assert self.tlvs == self.ins.tlvs
    def test_parser(self):
        ...
        assert self.transaction_id == res.transaction_id
        assert self.ttl == res.ttl
        assert self.relay_action == res.relay_action
        assert self.tlvs == res.tlvs
    def test_serialize(self):
        buf = self.ins.serialize()
        res = struct.unpack_from(self.form, bytes(buf))
        ...
        assert self.version == res[0] & 0x1f
        assert self.opcode == res[1]
        assert self.use_fdb_only == res[2] >> 7 & 0x01
        assert self.fwd_yes == res[2] >> 6 & 0x01
        assert self.terminal_mep == res[2] >> 5 & 0x01
        assert self.first_tlv_offset == res[3]
        assert self.transaction_id == res[4]
        assert self.ttl == res[5]
        assert self.relay_action == res[6]
        assert self.end_tlv == res[7]
    def test_len(self):
        # 11 octet (If tlv does not exist)
        assert 11 == len(self.ins)
    def test_default_args(self):
        ...
        assert res[1] == 4
        assert res[2] >> 7 == 1
        assert res[2] >> 6 & 0x01 == 0
        assert res[2] >> 5 & 0x01 == 1
        assert res[3] == 6
        assert res[4] == 0
        assert res[5] == 64
        assert res[6] == 1
class Test_sender_id_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.chassis_id_length == self.ins.chassis_id_length
        assert self.chassis_id_subtype == self.ins.chassis_id_subtype
        assert self.chassis_id == self.ins.chassis_id
        assert self.ma_domain_length == self.ins.ma_domain_length
        assert self.ma_domain == self.ins.ma_domain
        assert self.ma_length == self.ins.ma_length
        assert self.ma == self.ins.ma
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.length == res.length
        assert self.chassis_id_length == res.chassis_id_length
        assert self.chassis_id_subtype == res.chassis_id_subtype
        assert self.chassis_id == res.chassis_id
        assert self.ma_domain_length == res.ma_domain_length
        assert self.ma_domain == res.ma_domain
        assert self.ma_length == res.ma_length
    ...
    def test_serialize(self):
        ...
        assert self.chassis_id_subtype == res[3]
        assert self.chassis_id == res[4]
        assert self.ma_domain_length == res[5]
        assert self.ma_domain == res[6]
        assert self.ma_length == res[7]
        assert self.ma == res[8]
    ...
    def test_serialize_semi_normal_ptn2(self):
        ...
        assert 8 == res[1]
        assert 0 == res[2]
        assert self.ma_domain_length == res[3]
        assert self.ma_domain == res[4]
        assert self.ma_length == res[5]
        assert self.ma == res[6]
...
class Test_port_status_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.port_status == self.ins.port_status
    def test_parser(self):
...
class Test_data_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.data_value == self.ins.data_value
    def test_parser(self):
...
class Test_interface_status_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.interface_status == self.ins.interface_status
    def test_parser(self):
...
class Test_ltm_egress_identifier_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.egress_id_ui == self.ins.egress_id_ui
        assert self.egress_id_mac == self.ins.egress_id_mac
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.length == res.length
        assert self.egress_id_ui == res.egress_id_ui
    ...
    def test_serialize(self):
        ...
        assert addrconv.mac.text_to_bin(self.egress_id_mac) == res[3]
    def test_serialize_with_length_zero(self):
    ...
    def test_default_args(self):
        ...
        assert res[1] == 8
        assert res[2] == 0
        assert res[3] == addrconv.mac.text_to_bin('00:00:00:00:00:00')
class Test_ltr_egress_identifier_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.last_egress_id_ui == self.ins.last_egress_id_ui
        assert self.last_egress_id_mac == self.ins.last_egress_id_mac
        assert self.next_egress_id_ui == self.ins.next_egress_id_ui
        assert self.next_egress_id_mac == self.ins.next_egress_id_mac
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.length == res.length
        assert self.last_egress_id_ui == res.last_egress_id_ui
        assert self.last_egress_id_mac == res.last_egress_id_mac
        assert self.next_egress_id_ui == res.next_egress_id_ui
    ...
    def test_serialize(self):
        ...
        assert addrconv.mac.text_to_bin(self.last_egress_id_mac) == res[3]
        assert self.next_egress_id_ui == res[4]
        assert addrconv.mac.text_to_bin(self.next_egress_id_mac) == res[5]
    def test_serialize_with_length_zero(self):
    ...
    def test_default_args(self):
        ...
        assert res[3] == addrconv.mac.text_to_bin('00:00:00:00:00:00')
        assert res[4] == 0
        assert res[5] == addrconv.mac.text_to_bin('00:00:00:00:00:00')
class Test_organization_specific_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.oui == self.ins.oui
        assert self.subtype == self.ins.subtype
        assert self.value == self.ins.value
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.length == res.length
        assert self.oui == res.oui
        assert self.subtype == res.subtype
    ...
    def test_serialize(self):
        ...
        assert self.subtype == res[3]
        assert self.value == res[4]
...
class Test_reply_ingress_tlv(unittest.TestCase):
    ...
    def test_init(self):
        assert self.length == self.ins.length
        assert self.action == self.ins.action
        assert self.mac_address == self.ins.mac_address
        assert self.port_id_length == self.ins.port_id_length
        assert self.port_id_subtype == self.ins.port_id_subtype
        assert self.port_id == self.ins.port_id
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.length == res.length
        assert self.action == res.action
        assert self.mac_address == res.mac_address
        assert self.port_id_length == res.port_id_length
        assert self.port_id_subtype == res.port_id_subtype
    ...
    def test_serialize(self):
        ...
        assert addrconv.mac.text_to_bin(self.mac_address) == res[3]
        assert self.port_id_length == res[4]
        assert self.port_id_subtype == res[5]
        assert self.port_id == res[6]
    def test_serialize_with_zero(self):
