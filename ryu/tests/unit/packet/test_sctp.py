...
class Test_sctp(unittest.TestCase):
    ...
    def test_init(self):
        assert self.src_port == self.sc.src_port
        assert self.dst_port == self.sc.dst_port
        assert self.vtag == self.sc.vtag
        assert self.csum == self.sc.csum
        assert self.chunks == self.sc.chunks
    ...
    def test_parser(self):
        ...
        assert self.src_port == res.src_port
        assert self.dst_port == res.dst_port
        assert self.vtag == res.vtag
        assert self.csum == res.csum
        assert str(self.chunks) == str(res.chunks)
    ...
    def _test_serialize(self):
        ...
        assert self.src_port == res[0]
        assert self.dst_port == res[1]
        assert self.vtag == res[2]
    ...
    def test_serialize_with_data(self):
        ...
        assert sctp.chunk_data.chunk_type() == res[0]
        flags = (
            (self.unordered << 2) |
            (self.begin << 1) |
            (self.end << 0))
        assert flags == res[1]
        assert self.length == res[2]
        assert self.tsn == res[3]
        assert self.sid == res[4]
        assert self.seq == res[5]
        assert self.payload_id == res[6]
        assert self.payload_data == buf[sctp.chunk_data._MIN_LEN:]

    def test_serialize_with_init(self):
        self.setUp_with_init()
        buf = self._test_serialize()
        res = struct.unpack_from(sctp.chunk_init._PACK_STR, buf)
        assert sctp.chunk_init.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.init_tag == res[3]
        assert self.a_rwnd == res[4]
        assert self.os == res[5]
        assert self.mis == res[6]
        assert self.i_tsn == res[7]

        buf = buf[sctp.chunk_init._MIN_LEN:]
        res1 = struct.unpack_from(sctp.param_ipv4._PACK_STR, buf)
        assert sctp.param_ipv4.param_type() == res1[0]
        assert 8 == res1[1]
        assert '192.168.1.1' == (addrconv.ipv4.bin_to_text(
            buf[sctp.param_ipv4._MIN_LEN:sctp.param_ipv4._MIN_LEN + 4]))

        buf = buf[8:]
        res2 = struct.unpack_from(sctp.param_ipv6._PACK_STR, buf)
        assert sctp.param_ipv6.param_type() == res2[0]
        assert 20 == res2[1]
        assert 'fe80::647e:1aff:fec4:8284' == (addrconv.ipv6.bin_to_text(
            buf[sctp.param_ipv6._MIN_LEN:sctp.param_ipv6._MIN_LEN + 16]))

        buf = buf[20:]
        res3 = struct.unpack_from(sctp.param_cookie_preserve._PACK_STR,
                                  buf)
        assert sctp.param_cookie_preserve.param_type() == res3[0]
        assert 8 == res3[1]
        assert 5000 == res3[2]

        buf = buf[8:]
        res4 = struct.unpack_from(sctp.param_ecn._PACK_STR, buf)
        assert sctp.param_ecn.param_type() == res4[0]
        assert 4 == res4[1]

        buf = buf[4:]
        res5 = struct.unpack_from(sctp.param_host_addr._PACK_STR, buf)
        assert sctp.param_host_addr.param_type() == res5[0]
        assert 14 == res5[1]
        assert b'test host\x00' == (buf[sctp.param_host_addr._MIN_LEN:
                sctp.param_host_addr._MIN_LEN + 10])

        buf = buf[16:]
        res6 = struct.unpack_from(sctp.param_supported_addr._PACK_STR, buf)
        res6 = list(res6)
        assert sctp.param_supported_addr.param_type() == res6[0]
        assert 14 == res6[1]
        ...
        res6.extend(tmplist)
        assert sctp.PTYPE_IPV4 == res6[2]
        assert sctp.PTYPE_IPV6 == res6[3]
        assert sctp.PTYPE_COOKIE_PRESERVE == res6[4]
        assert sctp.PTYPE_ECN == res6[5]
        assert sctp.PTYPE_HOST_ADDR == res6[6]

    def test_serialize_with_init_ack(self):
        self.setUp_with_init_ack()
        buf = self._test_serialize()
        res = struct.unpack_from(sctp.chunk_init_ack._PACK_STR, buf)
        assert sctp.chunk_init_ack.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.init_tag == res[3]
        assert self.a_rwnd == res[4]
        ...
        assert sctp.param_state_cookie.param_type() == res1[0]
        assert 7 == res1[1]
        assert b'\x01\x02\x03' == (buf[sctp.param_state_cookie._MIN_LEN:
                sctp.param_state_cookie._MIN_LEN + 3])

        buf = buf[8:]
        res2 = struct.unpack_from(sctp.param_ipv4._PACK_STR, buf)
        assert sctp.param_ipv4.param_type() == res2[0]
        assert 8 == res2[1]
        assert '192.168.1.1' == (addrconv.ipv4.bin_to_text(
            buf[sctp.param_ipv4._MIN_LEN:sctp.param_ipv4._MIN_LEN + 4]))
        ...
        assert sctp.param_ipv6.param_type() == res3[0]
        assert 20 == res3[1]
        assert 'fe80::647e:1aff:fec4:8284' == (addrconv.ipv6.bin_to_text(
            buf[sctp.param_ipv6._MIN_LEN:sctp.param_ipv6._MIN_LEN + 16]))
        ...
        assert sctp.param_unrecognized_param.param_type() == res4[0]
        assert 8 == res4[1]
        assert b'\xff\xff\x00\x04' == (buf[sctp.param_unrecognized_param._MIN_LEN:
                sctp.param_unrecognized_param._MIN_LEN + 4])

        buf = buf[8:]
        res5 = struct.unpack_from(sctp.param_ecn._PACK_STR, buf)
        assert sctp.param_ecn.param_type() == res5[0]
        assert 4 == res5[1]

        buf = buf[4:]
        res6 = struct.unpack_from(sctp.param_host_addr._PACK_STR, buf)
        assert sctp.param_host_addr.param_type() == res6[0]
        assert 14 == res6[1]
        assert b'test host\x00' == (buf[sctp.param_host_addr._MIN_LEN:
                sctp.param_host_addr._MIN_LEN + 10])
    ...
    def test_serialize_with_sack(self):
        ...
        assert sctp.chunk_sack.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.tsn_ack == res[3]
        assert self.a_rwnd == res[4]
        assert self.gapack_num == res[5]
        assert self.duptsn_num == res[6]
        ...
        assert self.gapacks == gapacks
        assert self.duptsns == duptsns

    def test_serialize_with_heartbeat(self):
        self.setUp_with_heartbeat()
        buf = self._test_serialize()
        res = struct.unpack_from(sctp.chunk_heartbeat._PACK_STR, buf)
        assert sctp.chunk_heartbeat.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]

        buf = buf[sctp.chunk_heartbeat._MIN_LEN:]
        res1 = struct.unpack_from(sctp.param_heartbeat._PACK_STR, buf)
        assert sctp.param_heartbeat.param_type() == res1[0]
        assert 8 == res1[1]
        assert b'\x01\x02\x03\x04' == (buf[sctp.param_heartbeat._MIN_LEN:
    ...
    def test_serialize_with_heartbeat_ack(self):
        ...
        assert sctp.chunk_heartbeat_ack.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        ...
        assert sctp.param_heartbeat.param_type() == res1[0]
        assert 12 == res1[1]
        assert b'\xff\xee\xdd\xcc\xbb\xaa\x99\x88' == (buf[sctp.param_heartbeat._MIN_LEN:
    ...
    def test_serialize_with_abort(self):
        ...
        assert sctp.chunk_abort.chunk_type() == res[0]
        flags = self.tflag << 0
        assert flags == res[1]
        assert self.length == res[2]
        ...
        assert sctp.cause_invalid_stream_id.cause_code() == res1[0]
        assert 8 == res1[1]
        assert 4096 == res1[2]

        buf = buf[8:]
        res2 = struct.unpack_from(sctp.cause_missing_param._PACK_STR, buf)
        assert sctp.cause_missing_param.cause_code() == res2[0]
        assert 16 == res2[1]
        assert 4 == res2[2]
        types = []
        for count in range(4):
            ...
            types.append(tmp)
        assert str([sctp.PTYPE_IPV4, sctp.PTYPE_IPV6,
                 sctp.PTYPE_COOKIE_PRESERVE, sctp.PTYPE_HOST_ADDR]) == str(types)

        buf = buf[16:]
        res3 = struct.unpack_from(sctp.cause_stale_cookie._PACK_STR, buf)
        assert sctp.cause_stale_cookie.cause_code() == res3[0]
        assert 8 == res3[1]
        assert b'\x00\x00\x13\x88' == (buf[sctp.cause_stale_cookie._MIN_LEN:
                sctp.cause_stale_cookie._MIN_LEN + 4])

        buf = buf[8:]
        res4 = struct.unpack_from(sctp.cause_out_of_resource._PACK_STR, buf)
        assert sctp.cause_out_of_resource.cause_code() == res4[0]
        assert 4 == res4[1]

        buf = buf[4:]
        res5 = struct.unpack_from(
            sctp.cause_unresolvable_addr._PACK_STR, buf)
        assert sctp.cause_unresolvable_addr.cause_code() == res5[0]
        assert 20 == res5[1]
        assert (b'\x00\x0b\x00\x0e\x74\x65\x73\x74' +
            b'\x20\x68\x6f\x73\x74\x00\x00\x00') == (buf[sctp.cause_unresolvable_addr._MIN_LEN:
                sctp.cause_unresolvable_addr._MIN_LEN + 16])

        buf = buf[20:]
        res6 = struct.unpack_from(
            sctp.cause_unrecognized_chunk._PACK_STR, buf)
        assert sctp.cause_unrecognized_chunk.cause_code() == res6[0]
        assert 8 == res6[1]
        assert b'\xff\x00\x00\x04' == (buf[sctp.cause_unrecognized_chunk._MIN_LEN:
                sctp.cause_unrecognized_chunk._MIN_LEN + 4])

        buf = buf[8:]
        res7 = struct.unpack_from(sctp.cause_invalid_param._PACK_STR, buf)
        assert sctp.cause_invalid_param.cause_code() == res7[0]
        assert 4 == res7[1]

        buf = buf[4:]
        res8 = struct.unpack_from(
            sctp.cause_unrecognized_param._PACK_STR, buf)
        assert sctp.cause_unrecognized_param.cause_code() == res8[0]
        assert 8 == res8[1]
        assert b'\xff\xff\x00\x04' == (buf[sctp.cause_unrecognized_param._MIN_LEN:
                sctp.cause_unrecognized_param._MIN_LEN + 4])

        buf = buf[8:]
        res9 = struct.unpack_from(sctp.cause_no_userdata._PACK_STR, buf)
        assert sctp.cause_no_userdata.cause_code() == res9[0]
        assert 8 == res9[1]
        assert b'\x00\x01\xe2\x40' == (buf[sctp.cause_no_userdata._MIN_LEN:
                sctp.cause_no_userdata._MIN_LEN + 4])

        buf = buf[8:]
        res10 = struct.unpack_from(
            sctp.cause_cookie_while_shutdown._PACK_STR, buf)
        assert sctp.cause_cookie_while_shutdown.cause_code() == res10[0]
        assert 4 == res10[1]

        buf = buf[4:]
        res11 = struct.unpack_from(
            sctp.cause_restart_with_new_addr._PACK_STR, buf)
        assert sctp.cause_restart_with_new_addr.cause_code() == res11[0]
        assert 12 == res11[1]
        assert b'\x00\x05\x00\x08\xc0\xa8\x01\x01' == (buf[sctp.cause_restart_with_new_addr._MIN_LEN:
                sctp.cause_restart_with_new_addr._MIN_LEN + 8])

        buf = buf[12:]
        res12 = struct.unpack_from(
            sctp.cause_user_initiated_abort._PACK_STR, buf)
        assert sctp.cause_user_initiated_abort.cause_code() == res12[0]
        assert 19 == res12[1]
        assert b'Key Interrupt.\x00' == (buf[sctp.cause_user_initiated_abort._MIN_LEN:
                sctp.cause_user_initiated_abort._MIN_LEN + 15])

        buf = buf[20:]
        res13 = struct.unpack_from(
            sctp.cause_protocol_violation._PACK_STR, buf)
        assert sctp.cause_protocol_violation.cause_code() == res13[0]
        assert 20 == res13[1]
        assert b'Unknown reason.\x00' == (buf[sctp.cause_protocol_violation._MIN_LEN:
    ...
    def test_serialize_with_shutdown(self):
        ...
        assert sctp.chunk_shutdown.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.tsn_ack == res[3]
    ...
    def test_serialize_with_shutdown_ack(self):
        ...
        assert sctp.chunk_shutdown_ack.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
    ...
    def test_serialize_with_error(self):
        ...
        assert sctp.chunk_error.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
    ...
    def test_serialize_with_cookie_echo(self):
        ...
        assert sctp.chunk_cookie_echo.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.cookie == (buf[sctp.chunk_cookie_echo._MIN_LEN:
    ...
    def test_serialize_with_cookie_ack(self):
        ...
        assert sctp.chunk_cookie_ack.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
    ...
    def test_serialize_with_ecn_echo(self):
        ...
        assert sctp.chunk_ecn_echo.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.low_tsn == res[3]

    def test_serialize_with_cwr(self):
        self.setUp_with_cwr()
        buf = self._test_serialize()
        res = struct.unpack_from(sctp.chunk_cwr._PACK_STR, buf)
        assert sctp.chunk_cwr.chunk_type() == res[0]
        assert self.flags == res[1]
        assert self.length == res[2]
        assert self.low_tsn == res[3]
    ...
    def test_serialize_with_shutdown_complete(self):
        ...
        assert sctp.chunk_shutdown_complete.chunk_type() == res[0]
        flags = self.tflag << 0
        assert flags == res[1]
        assert self.length == res[2]
    ...
    def test_serialize_with_multi_chunks(self):
        ...
        res = struct.unpack_from(sctp.chunk_sack._PACK_STR, buf)
        assert sctp.chunk_sack.chunk_type() == res[0]
        assert self.s_flags == res[1]
        assert self.s_length == res[2]
        assert self.s_tsn_ack == res[3]
        assert self.s_a_rwnd == res[4]
        assert self.s_gapack_num == res[5]
        assert self.s_duptsn_num == res[6]
        ...
        d1_flags = (
            ...
            (self.d1_begin << 1) |
            (self.d1_end << 0))
        assert d1_flags == res[1]
        assert self.d1_length == res[2]
        assert self.d1_tsn == res[3]
        assert self.d1_sid == res[4]
        assert self.d1_seq == res[5]
        assert self.d1_payload_id == res[6]
        assert self.d1_payload_data == (buf[sctp.chunk_data._MIN_LEN:
        ...
        d2_flags = (
            ...
            (self.d2_begin << 1) |
            (self.d2_end << 0))
        assert d2_flags == res[1]
        assert self.d2_length == res[2]
        assert self.d2_tsn == res[3]
        assert self.d2_sid == res[4]
        assert self.d2_seq == res[5]
        assert self.d2_payload_id == res[6]
        assert self.d2_payload_data == (buf[sctp.chunk_data._MIN_LEN:
                sctp.chunk_data._MIN_LEN + 10])
    ...
    def test_build_sctp(self):
        ...
        assert eth
        assert eth.ethertype == ether.ETH_TYPE_IP

        ip4 = pkt.get_protocol(ipv4.ipv4)
        assert ip4
        assert ip4.proto == inet.IPPROTO_SCTP
    ...
    def test_to_string(self):
        ...
        assert str(self.sc) == sctp_str
        assert repr(self.sc) == sctp_str
