...
class Test_bgp(unittest.TestCase):
    ...
    def test_parser(self):
        ...
        for f in files:
            ...
            for _, buf in pcaplib.Reader(
                ...
                assert isinstance(pkt.protocols[-1], bgp.BGPMessage), 'Failed to parse BGP message: %s' % pkt
    ...
    def test_flowspec_user_interface_ipv4(self):
        ...
        assert str(msg) == str(msg2)
        assert binary_str(binmsg) == binary_str(binmsg2)
        msg3, rest = bgp.FlowSpecIPv4NLRI.parser(binmsg)
        assert str(msg) == str(msg3)
        assert rest == b''
