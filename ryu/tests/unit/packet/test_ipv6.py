...
class Test_ipv6(unittest.TestCase):
    ...
    def test_init(self):
        ...
        assert self.traffic_class == self.ip.traffic_class
        assert self.flow_label == self.ip.flow_label
        assert self.payload_length == self.ip.payload_length
        assert self.nxt == self.ip.nxt
        assert self.hop_limit == self.ip.hop_limit
        assert self.src == self.ip.src
        assert self.dst == self.ip.dst
        assert str(self.ext_hdrs) == str(self.ip.ext_hdrs)
    ...
    def test_parser(self):
        ...
        assert self.traffic_class == res.traffic_class
        assert self.flow_label == res.flow_label
        assert self.payload_length == res.payload_length
        assert self.nxt == res.nxt
        assert self.hop_limit == res.hop_limit
        assert self.src == res.src
        assert self.dst == res.dst
        assert str(self.ext_hdrs) == str(res.ext_hdrs)
    ...
    def test_serialize(self):
        ...
        assert self.nxt == res[2]
        assert self.hop_limit == res[3]
        assert self.src == addrconv.ipv6.bin_to_text(res[4])
        assert self.dst == addrconv.ipv6.bin_to_text(res[5])
    ...
    def test_to_string(self):
        ...
        assert str(self.ip) == ipv6_str
        assert repr(self.ip) == ipv6_str
    ...
    def test_len(self):
        assert len(self.ip) == 40
    def test_len_with_hop_opts(self):
        self.setUp_with_hop_opts()
        assert len(self.ip) == 40 + len(self.hop_opts)
    def test_len_with_dst_opts(self):
        self.setUp_with_dst_opts()
        assert len(self.ip) == 40 + len(self.dst_opts)
    def test_len_with_routing_type3(self):
        self.setUp_with_routing_type3()
        assert len(self.ip) == 40 + len(self.routing)
    def test_len_with_fragment(self):
        self.setUp_with_fragment()
        assert len(self.ip) == 40 + len(self.fragment)
    def test_len_with_auth(self):
        self.setUp_with_auth()
        assert len(self.ip) == 40 + len(self.auth)
    def test_len_with_multi_headers(self):
        self.setUp_with_multi_headers()
        assert len(self.ip) == 40 + len(self.hop_opts) + len(self.auth)
    def test_default_args(self):
        ...
        assert res[2] == 6
        assert res[3] == 255
        assert res[4] == addrconv.ipv6.text_to_bin('10::10')
        assert res[5] == addrconv.ipv6.text_to_bin('20::20')
        ...
        assert res[4] == addrconv.ipv6.text_to_bin('10::10')
        assert res[5] == addrconv.ipv6.text_to_bin('20::20')
        assert res[6] == b'\x3a\x00\x05\x02\x00\x00\x01\x00'
    def test_json(self):
...
class Test_hop_opts(unittest.TestCase):
    ...
    def test_serialize(self):
        ...
        assert 1 == opt2.type_
        assert 0 == opt2.len_
        assert None == opt2.data
        assert 0xc2 == opt3.type_
        assert 4 == opt3.len_
        assert b'\x00\x01\x00\x00' == opt3.data
        assert 1 == opt4.type_
        assert 0 == opt4.len_
        assert None == opt4.data
    def test_len(self):
        assert 16 == len(self.hop)
    def test_default_args(self):
        hdr = ipv6.hop_opts()
        buf = hdr.serialize()
...
class Test_option(unittest.TestCase):
    ...
    def test_init(self):
        assert self.type_ == self.opt.type_
        assert self.len_ == self.opt.len_
        assert self.data == self.opt.data
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.type_ == res.type_
        assert self.len_ == res.len_
        assert self.data == res.data
    def test_serialize(self):
        buf = self.opt.serialize()
        res = struct.unpack_from(self.form, buf)
        assert self.type_ == res[0]
        assert self.len_ == res[1]
        assert self.data == res[2]
    def test_len(self):
        assert len(self.opt) == 2 + self.len_
...
class Test_routing(unittest.TestCase):
    ...
    def test_parser(self):
        ...
        assert self.size == res.size
        assert self.type_ == res.type_
        assert self.seg == res.seg
        assert self.cmpi == res.cmpi
        assert self.cmpe == res.cmpe
        assert self.pad == res._pad
        assert self.adrs[0] == res.adrs[0]
        assert self.adrs[1] == res.adrs[1]
...
class Test_routing_type3(unittest.TestCase):
    ...
    def test_init(self):
        assert self.nxt == self.routing.nxt
        assert self.size == self.routing.size
        assert self.type_ == self.routing.type_
        assert self.seg == self.routing.seg
        assert self.cmpi == self.routing.cmpi
        assert self.cmpe == self.routing.cmpe
        assert self.pad == self.routing._pad
        assert self.adrs[0] == self.routing.adrs[0]
        assert self.adrs[1] == self.routing.adrs[1]
        assert self.adrs[2] == self.routing.adrs[2]
    def test_parser(self):
        _res = ipv6.routing.parser(self.buf)
        if type(_res) is tuple:
            res = _res[0]
    ...
    def test_serialize(self):
        ...
        assert self.seg == res[3]
        assert self.cmpi == res[4] >> 4
        assert self.cmpe == res[4] & 0xf
        assert self.pad == res[5]
        assert addrconv.ipv6.text_to_bin(self.adrs[0]) == res[6]
        assert addrconv.ipv6.text_to_bin(self.adrs[1]) == res[7]
        assert addrconv.ipv6.text_to_bin(self.adrs[2]) == res[8]
    def test_parser_with_adrs_zero(self):
        ...
        assert type_ == res.type_
        assert seg == res.seg
        assert cmpi == res.cmpi
        assert cmpe == res.cmpe
        assert pad == res._pad
    ...
    def test_parser_with_compression(self):
        ...
        assert cmpe == res.cmpe
        assert pad == res._pad
        assert "::4567:89ab:cdef:1" == res.adrs[0]
        assert "::4567:89ab:cdef:2" == res.adrs[1]
        assert "::205.239.0.3" == res.adrs[2]
    def test_serialize_with_compression(self):
        ...
        assert cmpe == res[4] & 0xf
        assert pad == res[5]
        assert addrconv.ipv6.text_to_bin(adrs[0])[slice_i] == res[6]
        assert addrconv.ipv6.text_to_bin(adrs[1])[slice_i] == res[7]
        assert addrconv.ipv6.text_to_bin(adrs[2])[slice_e] == res[8]
    def test_len(self):
        assert (6 + 1) * 8 == len(self.routing)
    def test_default_args(self):
...
class Test_fragment(unittest.TestCase):
    ...
    def test_init(self):
        assert self.nxt == self.fragment.nxt
        assert self.offset == self.fragment.offset
        assert self.more == self.fragment.more
        assert self.id_ == self.fragment.id_
    def test_parser(self):
        ...
        if type(_res) is tuple:
            ...
            res = _res
        assert self.nxt == res.nxt
        assert self.offset == res.offset
        assert self.more == res.more
...
class Test_auth(unittest.TestCase):
    ...
    def test_init(self):
        assert self.nxt == self.auth.nxt
        assert self.size == self.auth.size
        assert self.spi == self.auth.spi
        assert self.seq == self.auth.seq
        assert self.data == self.auth.data
    def test_parser(self):
        ...
        assert self.nxt == res.nxt
        assert self.size == res.size
        assert self.spi == res.spi
        assert self.seq == res.seq
        assert self.data == res.data
    def test_serialize(self):
        ...
        assert self.seq == res[3]
        assert self.data == res[4]
    def test_len(self):
        assert (4 + 2) * 4 == len(self.auth)
    def test_len_re(self):
        size = 5
        auth = ipv6.auth(
            0, size, 256, 1,
            b'\x21\xd3\xa9\x5c\x5f\xfd\x4d\x18\x46\x22\xb9\xf8\xf8\xf8\xf8\xf8')
        assert (size + 2) * 4 == len(auth)
    def test_default_args(self):
        hdr = ipv6.auth()
        buf = hdr.serialize()
        LOG.info(repr(buf))
        ...
        assert res[2] == 0
        assert res[3] == 0
        assert buf[ipv6.auth._MIN_LEN:] == b'\x00\x00\x00\x00'